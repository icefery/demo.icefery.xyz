...
```shell
# 硬编码请求体
curl 'http://localhost:8080/api' -H 'content-type: application/json' --data-raw '{"name":"john","age":30}'

# 读文件请求体
cat data.json | curl 'http://localhost:8080/api' -H 'conctent-type: application/json' -d @-
```
...
```shell
# 自动处理编码
curl 'http://localhost:8080/api' -d 'name=John Doe' -d 'email=join@example.com'

# 手动处理编码
curl 'http://localhost:8080/api' --data-raw 'name=John%20Doe&email=john%40example.com'
```

#### 发送 `multipart/form-data`

```shell
curl 'http://localhost:8080/api' --form "username=john" --form "password=secret" --form "avatar=@profile.jpg"
```

  ...
  <view class="u-margin-left-20 u-margin-right-20">
    <u-form>
      <u-divider half-width="100%"
        ><u-tag text="下拉拉取" size="mini"
      /></u-divider>
      <u-form-item
        ><u-input
          :border="true"
          v-model="questionnaireItem.tenantId"
          placeholder="租户号"
          input-align="center"
      /></u-form-item>
      <u-form-item
        ><u-input
          :border="true"
          v-model="questionnaireItem.questionnaireId"
          placeholder="问卷号"
          input-align="center"
      /></u-form-item>
      ...
      <template v-for="(question, index) in questionnaireItem.questions">
        <u-divider half-width="100%"
          ><u-tag size="mini" :text="index + 1 + ' ' + question.title"
        /></u-divider>
        ...
        <u-form-item>
          ...
          <template v-else-if="question.type === 'location'">
            <u-button
              style="width: 100%"
              @click="() => (question.control.pickerShow = true)"
            >
          ...
          <template v-else-if="question.type === 'datetime'">
            <u-button
              style="width: 100%"
              @click="() => (question.control.pickerShow = true)"
              >{{ computedDatetimeDisplayValue(question) }}</u-button
            >
            <u-picker
              v-model="question.control.pickerShow"
              mode="time"
    ...
    computedPickerParam() {
      return (params) => {
        ...
        params
          ...
          .forEach((item) => (obj[item.value] = true));
        return obj;
      };
    },
  },

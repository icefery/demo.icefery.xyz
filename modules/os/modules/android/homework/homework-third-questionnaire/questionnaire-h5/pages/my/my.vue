  ...
  <view class="u-margin-left-20 u-margin-right-20">
      ...
      <u-form>
        ...
        <u-form-item>
          <u-button
            ...
            :disabled="tenant.username === '' || tenant.password === ''"
            @click="doLogin()"
          >
            登录
          </u-button>
    ...
    <template v-else>
      <u-table class="u-margin-top-20 u-margin-bottom-10">
        <u-tr>
          <u-td
            ><u-tag :text="login.id" @click="() => doCopy(login.id)"
          /></u-td>
          <u-td
            ><u-tag
              :text="login.username"
              @click="() => doCopy(login.username)"
          /></u-td>
        </u-tr>
      </u-table>

      <u-button
        class="u-margin-top-10 u-margin-bottom-10"
        style="width: 100%"
        @click="doLogout()"
        size="mini"
        >注销</u-button
        ...
        <u-table class="u-margin-top-10">
          ...
          <template v-for="quesionnaire in quesionnaires">
            <u-tr>
              <u-td
                ><u-tag
                  :text="quesionnaire.id"
                  @click="() => doCopy(quesionnaire.id)"
              /></u-td>
              <u-td
                ><u-tag
                  :text="quesionnaire.title"
                  @click="() => doCopy(quesionnaire.title)"
              /></u-td>
            </u-tr>
          </template>
        </u-table>
